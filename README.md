...
# 6. 启动 Prometheus 和 Grafana 监控服务（Docker Compose）
docker compose -f docker-compose-monitor.yml up -d

# 7. 等待数据库和服务启动完成（约 5-10 秒）
timeout /t 10
...
.venv\Scripts\Activate.ps1; python -m uvicorn app.main:app --host 0.0.0.0 --port 9900
...
使用全新的 PowerShell 启动脚本（自动启动所有服务并打开浏览器）：

```powershell
# 启动所有服务（数据库 + MCP服务 + 后端 + 前端）
.\start-all.ps1

# 查看后台服务的日志
.\view-logs.ps1

# 启动服务并显示实时日志（顺序启动，无法同时管理）
.\start-all.ps1 -ShowLogs
