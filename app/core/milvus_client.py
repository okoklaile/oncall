...
class MilvusClientManager:
    ...
    def connect(self) -> MilvusClient:
        ...
        try:
            ...
            if not self._collection_exists():
                ...
                # 检查 Schema 是否支持 BM25 (是否存在 sparse_vector 字段)
                schema = self._collection.schema
                has_sparse = any(f.name == "sparse_vector" for f in schema.fields)
                
                # 检查向量维度是否匹配
                vector_field = next((f for f in schema.fields if f.name == "vector"), None)
                existing_dim = vector_field.params.get('dim') if vector_field and hasattr(vector_field, 'params') else None
                
                if not has_sparse or existing_dim != self.VECTOR_DIM:
                    reason = "缺少 sparse_vector 字段 (BM25)" if not has_sparse else f"维度不匹配 ({existing_dim} != {self.VECTOR_DIM})"
                    logger.warning(f"检测到 Collection Schema 需要更新: {reason}")
                    ...
                    self._create_collection()
                    logger.info("成功重新创建 collection")
                else:
                    logger.info("Collection Schema 验证通过 (已支持 BM25)")
    ...
    def _create_collection(self) -> None:
        ...
        # 定义 BM25 函数，自动从 content 生成 sparse_vector
        ...
        schema = CollectionSchema(
            ...
            description="Business knowledge collection with BM25 support",
