...
作用:
- 提供底层向量相似度搜索能力，直接在 Milvus collection 上执行 ANN 检索。

前面是谁:
- 上游是业务侧查询请求方（当前主对话链路中较少直接使用）。

后面是谁:
- 下游先调用 vector_embedding_service 生成 query 向量，再调用 milvus_manager.get_collection().search。
"""
