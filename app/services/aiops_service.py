...
class AIOpsService:
    ...
    def __init__(self):
        ...
        try:
            ...
            logger.info(f"AIOps AsyncRedis Checkpointer 初始化完成: {config.redis_url}")
        except Exception as e:
            logger.error(f"AIOps AsyncRedis Checkpointer 初始化失败，回退到内存模式: {e}")
    ...
    async def _initialize_checkpointer(self):
        ...
        if isinstance(self.checkpointer, AsyncRedisSaver):
            try:
                ...
                logger.info("AIOps AsyncRedis Checkpointer setup 成功")
            except Exception as e:
                logger.error(f"AIOps AsyncRedis Checkpointer setup 失败: {e}")
    ...
    def _build_graph(self):
        ...
        workflow.add_node(NODE_EXECUTOR, self._run_executor1)  # 执行步骤
    ...
    async def _run_executor1(self, state: PlanExecuteState, config: RunnableConfig):
        """包装 executor1，注入 checkpointer 并透传 config"""
        return await executor1(state, config, checkpointer=self.checkpointer)
    ...
    async def diagnose(
        ...
        aiops_task = dedent("""诊断当前系统是否存在告警，如果存在告警请详细分析告警原因并生成诊断报告，诊断报告输出格式要求：
                ...
                **重要提醒**：
                ...
                - 总共有三个topic，分别为topic-001，topic-002，topic-003
                """)
