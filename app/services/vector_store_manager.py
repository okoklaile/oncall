...
作用:
- 封装 LangChain Milvus VectorStore 的增删查能力，统一 collection 与字段映射。

前面是谁:
- 上游包括 vector_index_service（入库/删旧）与 knowledge_tool（检索）。

后面是谁:
- 下游是 Milvus；入库时会通过 embedding_function 调用 vector_embedding_service 生成向量。
"""
